...
isAIstarting = bool(arguments[3])

if len(arguments) == 5:
    game = gamegui.Game(size, winning_condition, isAIstarting = True, ai_mode=arguments[4])
elif len(arguments) == 6:
    assert arguments[4] in ["network", "tree", "random"], \
        "valid options for ai_mode are random, network or tree"

    game = gamegui.Game(size, winning_condition,isAIstarting = True, ai_mode=arguments[4], number_of_rollouts = int(arguments[5]))
else:
    game = gamegui.Game(size, winning_condition, isAIstarting = True)
